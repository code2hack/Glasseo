...
class DraftFoundationInstrumentationTest {
    @Before fun resetProbe() = ProbeState.reset()

    @Test fun draftStorageRoutingAndLayoutPassOnThePhysicalDisplay() {
        ActivityScenario.launch(MainActivity::class.java).use { scenario ->
            ...
            assertEquals(18, result.getInt("restoredTextLength"))
            assertEquals(8, result.getInt("restoredTextOffset"))
            assertTrue(result.getBoolean("restoredRequestCursor"))
            assertEquals(1, result.getInt("restoredImageCount"))
            assertTrue(result.getBoolean("betaBlank"))
            assertTrue(result.getBoolean("siblingBlank"))
            assertTrue(result.getBoolean("stableArea"))
            assertTrue(result.getBoolean("forwardedCommand"))
            assertTrue(result.getBoolean("structuredRecordOnly"))
    ...
    private fun awaitResult(scenario: ActivityScenario<MainActivity>): JSONObject {
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            scenario.onActivity { activity ->
                activity.evaluateJavascriptForTest("document.body.dataset.draftUi || ''") {
        ...
        throw AssertionError("Draft UI acceptance did not report")
    }

    companion object {
        private val SCRIPT = """
            import('./draft/ui-acceptance-entry.js')
              .then(() => window.__glasseoDraftUiAcceptance.run())
              .then(result => document.body.dataset.draftUi = 'ok:' + JSON.stringify(result))
              .catch(error => document.body.dataset.draftUi = 'error:' + String(error))
        """.trimIndent()
    }
}
